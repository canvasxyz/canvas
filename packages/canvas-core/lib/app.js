...
import { getColumnType } from "./models.js";
import { actionType, actionPayloadType, sessionType, sessionPayloadType, } from "./actions.js";
...
export class App {
    constructor(multihash, database, feed, worker, actionPort, modelPort, routes, models, actionParameters, handle) {
        ...
        this.handle = handle;
        this.statements = { routes: {}, models: {} };
        this.callPool = new Map();
        ...
        tables.push("CREATE TABLE IF NOT EXISTS _sessions " +
        ...
        for (const [name, model] of Object.entries(this.models)) {
            ...
            const condition = (n) => `${n} = CASE WHEN timestamp < :timestamp THEN :${n} ELSE ${n} END`;
            const updates = keys.map(condition).join(", ");
            this.statements.models[name] = {
                set: this.database.prepare(`INSERT INTO ${name} (id, ${fields}) VALUES (:id, ${params}) ON CONFLICT (id) DO UPDATE SET ${updates}`),
            };
        }
    ...
    static async initialize(options) {
        ...
        return new App(options.multihash, database, feed, worker, actionChannel.port2, modelChannel.port2, routes, models, actionParameters, handle);
    }
    handleModelMessage(message) {
        ...
        try {
            ...
            call.reject(err instanceof Error ? err : new Error(err.toString()));
        }
    }
    ...
    async stop() {
        ...
        await new Promise((resolve, reject) => this.feed.close((err) => (err ? reject(err) : resolve())));
