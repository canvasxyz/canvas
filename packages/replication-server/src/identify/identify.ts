...
export class Identify implements Startable, IdentifyInterface {
	...
	async pushToConnections(connections: Connection[]): Promise<void> {
		...
		const agentVersion = uint8ArrayToString(
			peer.metadata.get("AgentVersion") ?? uint8ArrayFromString(this.host.agentVersion),
		)
		const protocolVersion = uint8ArrayToString(
			peer.metadata.get("ProtocolVersion") ?? uint8ArrayFromString(this.host.protocolVersion),
		)
		...
		const pushes = connections.map(async (connection) => {
			...
			try {
				...
				// only advertise the first 90 /gossiplog/* protocols
				const gossiplogProtocols = supportedProtocols.filter((p) => p.startsWith("/gossiplog"))
				const otherProtocols = supportedProtocols.filter((p) => !p.startsWith("/gossiplog"))
				const filteredSupportedProtocols = otherProtocols.concat(gossiplogProtocols.slice(0, 90))

				await pb.write(
					{
						listenAddrs: listenAddresses.map((ma) => ma.bytes),
						signedPeerRecord: signedPeerRecord.marshal(),
						protocols: filteredSupportedProtocols,
						agentVersion,
						protocolVersion,
					},
					{
				...
				if (existingPeer.peerRecordEnvelope != null) {
					...
					if (storedRecord.seqNumber >= peerRecord.seqNumber) {
						this.log(
							...
							storedRecord.seqNumber,
							peerRecord.seqNumber,
						)
