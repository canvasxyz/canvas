...
export class SIWFSigner extends AbstractSessionSigner<SIWFSessionData> {
	...
	public constructor({ sessionDuration, privateKey, ...init }: SIWFSignerInit = {}) {
		super("signer-ethereum-farcaster", ed25519, { sessionDuration: sessionDuration ?? 14 * DAYS })

		this.chainId = 10
		this.key = `signer-ethereum-farcaster`
		...
		if (privateKey) {
			...
			this.target.set(`canvas:signers/signer-ethereum-farcaster/seed`, privateKey)
		}
	}
