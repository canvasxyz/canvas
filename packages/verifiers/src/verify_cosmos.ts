...
// TODO: use bech32Prefixes from the core library
...
export const verifyCosmosActionSignature = async (action: Action): Promise<string> => {
	...
	const generatedAddressWithChainPrefix = pubkeyToAddress(stdSignature.pub_key, bech32Prefix)
	const actionToken = makeActionToken(action.payload)

	let isValid: boolean
	try {
		const generatedSignDoc = validationTokenToSignDoc(
			Buffer.from(JSON.stringify(actionToken)),
			generatedAddressWithChainPrefix
		)
	...
	if (isValid) {
		return generatedAddressWithChainPrefix
	} else {
		return ""
	}
}
...
export const verifyCosmosSessionSignature = async (session: Session): Promise<string> => {
	...
	const sessionToken = makeSessionToken(session.payload)

	let isValid: boolean
	try {
		// Generate sign doc from token and verify it against the signature
		const generatedSignDoc = validationTokenToSignDoc(
			Buffer.from(JSON.stringify(sessionToken)),
			generatedAddressWithCosmosPrefix
		)
		const generatedSignDoc_ = validationTokenToSignDoc(Buffer.from(JSON.stringify(sessionToken)), generatedAddress)

		const { pubkey, signature } = decodeSignature(stdSignature)
		const secpSignature = Secp256k1Signature.fromFixedLength(signature)
		const messageHash = new Sha256(serializeSignDoc(generatedSignDoc_)).digest()
		isValid = await Secp256k1.verifySignature(secpSignature, messageHash, pubkey)
		if (!isValid) {
			console.error("Signature mismatch.")
		}
