...
export const Request = $root.Request = (() => {
    ...
    Request.verify = function verify(message) {
        ...
        if (message.getValues != null && message.hasOwnProperty("getValues")) {
            ...
            properties.request = 1;
            {
                let error = $root.Request.GetValuesRequest.verify(message.getValues);
                if (error)
    ...
    Request.GetValuesRequest = (function() {
        ...
        GetValuesRequest.decode = function decode(reader, length) {
            if (!(reader instanceof $Reader))
                reader = $Reader.create(reader);
            let end = length === undefined ? reader.len : reader.pos + length, message = new $root.Request.GetValuesRequest();
            while (reader.pos < end) {
                let tag = reader.uint32();
                switch (tag >>> 3) {
                case 2: {
        ...
        GetValuesRequest.fromObject = function fromObject(object) {
            if (object instanceof $root.Request.GetValuesRequest)
                return object;
            let message = new $root.Request.GetValuesRequest();
            if (object.nodes) {
...
export const Response = $root.Response = (() => {
    ...
    Response.verify = function verify(message) {
        ...
        if (message.getValues != null && message.hasOwnProperty("getValues")) {
            ...
            properties.response = 1;
            {
                let error = $root.Response.GetValuesResponse.verify(message.getValues);
                if (error)
                    return "getValues." + error;
            }
        }
    ...
    Response.GetValuesResponse = (function() {
        ...
        GetValuesResponse.decode = function decode(reader, length) {
            if (!(reader instanceof $Reader))
                reader = $Reader.create(reader);
            let end = length === undefined ? reader.len : reader.pos + length, message = new $root.Response.GetValuesResponse();
            while (reader.pos < end) {
                let tag = reader.uint32();
                switch (tag >>> 3) {
                case 2: {
        ...
        GetValuesResponse.fromObject = function fromObject(object) {
            if (object instanceof $root.Response.GetValuesResponse)
                return object;
            let message = new $root.Response.GetValuesResponse();
            if (object.values) {
