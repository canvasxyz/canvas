...
export const ContractView: React.FC<{ topic: string; contract: string }> = ({ topic, contract }) => {
	...
	const start = useCallback(async () => {
		const res = await fetch(`${host}/api/apps/${topic}/start`, { method: "post" })
		if (res.status !== 200) {
	...
	const stop = useCallback(async () => {
		const res = await fetch(`${host}/api/apps/${topic}/stop`, { method: "post" })
		if (res.status !== 200) {
			const message = await res.text()
	...
	return (
		<div className="h-full p-2 flex flex-col gap-2">
			...
			<div className="flex-1">
				<Editor key={`${topic}/${status}`} initialValue={contract} readOnly={true} />
			</div>
		</div>
	)
}
