...
testPlatforms(
	"apply should roll back partially performed updates if it fails",
	async (t, openDB) => {
	...
	// We exclude durable-objects since the DO has its own guarantees
	// about transactionality that we can't really test.
	// See the note in packages/modeldb-durable-objects/src/ModelDB.ts
	{ sqlite: true, sqliteWasm: true, expo: true, pg: true, idb: true },
)
