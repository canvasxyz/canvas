...
export type FetchDiscoveryServiceComponents = {
...
export interface FetchDiscoveryServiceInit {
	resultLimit?: number
}

export class FetchDiscoveryService implements Startable {
	...
	constructor(private readonly components: FetchDiscoveryServiceComponents, init: FetchDiscoveryServiceInit) {
		this.log = logger(`canvas:discovery:fetch`)
	...
	private handlePeerUpdate = ({ detail: { peer, previous } }: CustomEvent<PeerUpdate>) => {
		...
		const topics = new Set(
			peer.protocols
				.filter((protocol) => protocol.startsWith(FetchDiscoveryService.protocolPrefix))
				.map((protocol) => protocol.slice(FetchDiscoveryService.protocolPrefix.length, protocol.lastIndexOf("/"))),
		)
		...
		const previousTopics = previous?.protocols
			.filter((protocol) => protocol.startsWith(FetchDiscoveryService.protocolPrefix))
			.map((protocol) => protocol.slice(FetchDiscoveryService.protocolPrefix.length))
...
export const fetchDiscovery =
	(init: FetchDiscoveryServiceInit = {}) =>
	(components: FetchDiscoveryServiceComponents) =>
		new FetchDiscoveryService(components, init)
