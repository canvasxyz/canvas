...
export class KeplrWebWalletConnector implements Connector {
	...
	constructor() {
		...
		this.chainSettings = {
			...
			meta: {
				...
				default_symbol: "OSMO",
				node: {
					altWalletUrl: "https://lcd-osmosis.blockapsis.com",
				},
			},
		}
	}
	...
	async enable({ onAccountsChanged }: { onAccountsChanged: (accounts: string[]) => void }): Promise<void> {
		...
		const client = await StargateClient.connect(this.chainSettings.rpc)
		...
		try {
			...
			const info: ChainInfo = {
				...
				rest: this.chainSettings.meta.node.altWalletUrl || this.chainSettings.rpc,
				bip44: {
					coinType: 118,
				},
				bech32Config: {
	...
	async createSessionSigner(account: string): Promise<SessionSigner> {
		...
		console.log(`creating session signer ${account}`)
...
export class KeplrWebWalletSessionSigner implements SessionSigner {
	...
	async signSessionPayload(payload: SessionPayload): Promise<Session> {
		...
		const stdSignature = await keplr.signArbitrary(chainId, await this.getAddress(), JSON.stringify(sessionToken))
		const signature = JSON.stringify(stdSignature)
		return { signature, payload, type: "session" }
	}
