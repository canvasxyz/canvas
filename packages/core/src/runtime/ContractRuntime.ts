				...
				return vm.wrapFunction(async function (this: QuickJSHandle, ...args: JSValue[]) {
					...
					const actionContext: ActionContext<any> = {
						db: {
							...
							link: async (model, propertyName, source, target) =>
								await self.context.linkModelValue(model, propertyName, source, target, self.#transaction),
							unlink: async (model, propertyName, source, target) =>
								await self.context.unlinkModelValue(model, propertyName, source, target, self.#transaction),
