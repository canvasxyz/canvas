...
file containing the contract exports. Type annotations are optional,
but helpful for IDE autocomple.
...
Now, we can import this contract from our frontend. We initialize the
`useCanvas` hook with a `null` first parameter, instead of with a
WebSocket URL.

As a result, the application will only run in the browser, and will
persist until IndexedDB is cleared, which may happen without the user
being notified.
...
So far, our development replicas have all been ephemeral. Data
will be cleared when the CLI is shut down (e.g. with Ctrl+C).
...
directory that you've provided, using SQLite.
...
keep the data from the existing app as a "snapshot", or to throw it
out entirely.
...
provide a contract. This will cause the local application to download
the latest snapshot and contract from the remote application.
...
If we don't need server-to-server libp2p sync for our applications, then
...
For deploying with server-to-server libp2p sync, your steps will vary
...
variable, and also configure the ANNOUNCE environment variable
so your node can tell other nodes how to reach it.
