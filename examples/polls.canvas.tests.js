;(async () => {
	...
	const signAndPost = async (call, args) => {
		...
		const ret = (
			await axios.post(`${ROOT_PATH}/${call}`, {
				from: signer.address,
				signature,
				data: message,
			})
	...
	const pollId = await signAndPost("createPoll", {
		title: "Should Verses adopt a motto?",
	})
	const cardId1 = await signAndPost("createCard", {
		pollId,
		text: "Yes, we should vote on one now",
	})
	const cardId2 = await signAndPost("createCard", {
		pollId,
		text: "Yes, with modifications to the question",
	})
	const cardId3 = await signAndPost("createCard", {
		pollId,
		text: "No, we should leave it open",
	})
