...
export const actions = {
	async thread({ community, title, body, link, topic }: { community: string, title: string, body: string, link: string, topic: number }) {
		const { db, did, id, timestamp } = this
	...
	async updateThread({ thread_id, title, body, link, topic }: { thread_id: string, title: string, body: string, link: string, topic: number }) {
		const { db, did, timestamp } = this
	...
	async comment({ thread_id, body, parent_comment_id }: { thread_id: string, body: string, parent_comment_id: string }) {
		const { db, did, id, timestamp } = this
