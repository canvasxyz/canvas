...
export async function handleSession(core: Core, req: NextApiRequest, res: NextApiResponse) {
...
export async function handleAction(core: Core, req: NextApiRequest, res: NextApiResponse) {
...
export async function handleRoute(core: Core, pathComponents: string[], req: NextApiRequest, res: NextApiResponse) {
	...
	if (req.headers.accept === "text/event-stream") {
		...
		try {
			const data = core.getRoute(route, params)
			res.status(StatusCodes.OK).json(data)
		} catch (err) {
			const message = err instanceof Error ? err.message : err
			res.status(StatusCodes.BAD_REQUEST).end(message)
		}
	}
}
